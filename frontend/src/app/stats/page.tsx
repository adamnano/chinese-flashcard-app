  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-1 mt-2 text-xs text-gray-400 dark:text-gray-500">
        <span>Less</span>
        {["bg-gray-100 dark:bg-gray-800", "bg-green-100 dark:bg-green-900", "bg-green-300 dark:bg-green-700", "bg-green-500", "bg-green-700 dark:bg-green-400"].map((c, i) => (
...
function LevelBar({ label, count, max }: { label: string; count: number; max: number }) {
  ...
  return (
    <div className="flex items-center gap-3 text-sm">
      <span className="w-16 text-right text-gray-500 dark:text-gray-400 text-xs flex-shrink-0">{label}</span>
      <div className="flex-1 bg-gray-100 dark:bg-gray-800 rounded-full h-3">
        <div className="bg-red-400 dark:bg-red-500 h-3 rounded-full" style={{ width: `${pct}%` }} />
      </div>
  ...
  return (
    <div className="max-w-3xl space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "Total Cards", value: stats.total_cards, color: "text-gray-800 dark:text-gray-200" },
          { label: "Due Today", value: stats.due_today, color: "text-orange-600 dark:text-orange-400" },
          { label: "Mastered", value: stats.mastered_cards, color: "text-green-600 dark:text-green-400" },
          { label: "Streak", value: `${stats.streak_days}d`, color: "text-red-600 dark:text-red-400" },
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-5">
          ...
          <div className="space-y-2">
            {[1, 2, 3, 4, 5, 6, 7].map((l) => {
              ...
              return <LevelBar key={l} label={l === 7 ? "7-9" : `HSK ${l}`} count={count} max={maxHsk} />;
            })}
            <LevelBar label="Unknown" count={stats.hsk_distribution.find((d) => d.level === null)?.count ?? 0} max={maxHsk} />
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-5">
          ...
          <div className="space-y-2">
            ...
            <LevelBar label="Unknown" count={stats.tocfl_distribution.find((d) => d.level === null)?.count ?? 0} max={maxTocfl} />
          </div>
        </div>
      </div>
      ...
      {sourceStats.length > 0 && (
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden">
          <h2 className="text-sm font-semibold p-4 border-b border-gray-200 dark:border-gray-800">By Source</h2>
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {sourceStats.map((s) => (
                <tr key={s.source_id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                  <td className="px-4 py-2 cjk">{s.title}</td>
                  <td className="px-4 py-2 text-right">{s.total_cards}</td>
                  <td className="px-4 py-2 text-right text-green-600 dark:text-green-400">{s.mastered_cards}</td>
                  <td className="px-4 py-2 text-right text-orange-600 dark:text-orange-400">{s.due_today}</td>
                </tr>
              ))}
            </tbody>
          </table>
